...
######################################################################
######################################################################
    X = data[["PIT_Entries", "CS_cacheHits", "Out_Interests", "Satisfied_Interests"]]
    y = data["Flow_Type"]
    X = X.dropna()
    y = y.dropna()
# 将数据分为训练集和测试集    
...
# 创建模型对象并训练    
    model = XGBClassifier(objective="binary:logistic", max_depth=30, gamma=0.1, node_size=3, learning_rate=0.01)
...
# 评估模型性能    
    ...
    plt.plot(accuracy, [0, 1], 'r--', linewidth=2)
    plt.plot(recall, [0, 1], 'g--', linewidth=2)
    plt.plot(f1, [0, 1], 'b--', linewidth=2)
    plt.xlabel('True')
